    ...
    switch (input.fPort) {
       case 6:
            if (input.bytes[2] === 0x00)
            {
                ...
                data.SWver = ((input.bytes[3] >> 4) & 0x0F) + ((input.bytes[3] & 0x0F) / 10);
           ...
           data.Device = getDeviceName(input.bytes[1]);
            ...
            if(input.bytes[2] === 0x01){
                ...
                data.PM0_3UM = (input.bytes[4]<<16 | input.bytes[5]<<8 | input.bytes[6]) + 10;
                if (data.PM0_3UM & 0x80){
                    data.PM0_3UM = data.PM0_3UM & 0x7F;
                }
                data.PM0_5UM = input.bytes[7];
                data.PM1_0UM = input.bytes[9];
                ...
                data.TemperaturewithPH = (input.bytes[6]<<8 | input.bytes[7])*0.01;
                data.ORP = (input.bytes[8]<<8 | input.bytes[9]);
                if (data.TemperaturewithPH & 0x80){
                    data.TemperaturewithPH = data.TemperaturewithPH & 0x7F;
                }
            }else if(input.bytes[2] === 0x09){
                data.NTU = (input.bytes[4]<<8 | input.bytes[5])*0.1;
                data.TemperaturewithNTU = (input.bytes[6]<<8 | input.bytes[7])*0.01;
                data.Soil_VWC = (input.bytes[8]<<8 | input.bytes[9])*0.01;
                if (data.TemperaturewithNTU & 0x80){
                    data.TemperaturewithNTU= data.TemperaturewithNTU & 0x7F;
                }
                ...
                data.WaterLevel = (input.bytes[8]<<8 | input.bytes[9]);
                data.Soil_EC = input.bytes[10]*0.1;
            }else if(input.bytes[2] === 0x0B){
                data.TemperaturewithLDO = (input.bytes[4]<<8 | input.bytes[5])*0.01;
                data.LDODOValue = (input.bytes[6]<<8 | input.bytes[7])*0.01;
                data.LDOSatValue = (input.bytes[8]<<8 | input.bytes[9])*0.1;
                if (data.TemperaturewithLDO & 0x80){
                    data.TemperaturewithLDO= data.TemperaturewithLDO & 0x7F;
                }
            }else if(input.bytes[2] === 0x0C){
                data.Temperature_2Bytes_0_01 = (input.bytes[4]<<8 | input.bytes[5])*0.01;
                data.Humidity_2Byte_0_01 = (input.bytes[6]<<8 | input.bytes[7])*0.01;
        ...
        case 7:
            ...
            || input.bytes[0] === getCmdToID("SoilCalibrateRsp"))
            {
                ...
                data.LDOsAltiud = (input.bytes[2]<<8 | input.bytes[3]) / 100;
                data.LDOsPSU = (input.bytes[4]<<8 | input.bytes[5]) / 100;
...
function encodeDownlink(input) {
    ...
    {
        var LDOsAltiud = input.data.LDOsAltiud * 256;
        var LDOsPSU = input.data.LDOsPSU * 256;
        ret = ret.concat(getCmdID, devid, (LDOsAltiud & 0xFF), ((LDOsAltiud >> 8) & 0xFF), (LDOsPSU & 0xFF), ((LDOsPSU >> 8) & 0xFF), 0x00, 0x00, 0x00, 0x00, 0x00);
    ...
    {
        var SoilType = input.data.SoilType;
        ret = ret.concat(getCmdID, devid, SoilType, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00);
...
function decodeDownlink(input) {
    ...
    switch (input.fPort) {
        case 7:
            ...
            {
                data.LDOsAltiud = (input.bytes[3]<<8 | input.bytes[2]) / 256;
                data.LDOsPSU = (input.bytes[5]<<8 | input.bytes[4]) / 256;
